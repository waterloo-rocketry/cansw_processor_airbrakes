...
static SemaphoreHandle_t ICMMutex;

// OTITS TESTS
Otits_Result_t test_ICMSanity() {
	Otits_Result_t res;
    if (xSemaphoreTake(ICMMutex, MUTEX_WAIT) != pdTRUE) {
        res.info = "mutex timeout";
...
bool ICM_20948_init() {
    ...
    res &= otitsRegister(test_ICMSanity, TEST_SOURCE_ICM, "ICMSanity");
    res &= otitsRegister(test_magSanity, TEST_SOURCE_ICM, "MagSanity");
